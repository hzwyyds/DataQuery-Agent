...
class Repository:
    ...
    async def update_workspace(self, workspace_id: str, name: str) -> dict:
        async with self.connect() as connection:
            cursor = await connection.execute(
    ...
    async def list_recent_completed_runs(
        self, workspace_id: str, current_run_id: str = "", limit: int = 5
    ) -> list[dict]:
        async with self.connect() as connection:
            cursor = await connection.execute(
                """SELECT * FROM runs
                   WHERE workspace_id = ? AND status = 'COMPLETED' AND id <> ?
                   ORDER BY created_at DESC LIMIT ?""",
                (workspace_id, current_run_id, min(max(limit, 1), 5)),
            )
